...
import { ArtelLogo } from "@/components/brand/ArtelLogo";

const DOT = ["var(--color-artel)", "var(--color-amber)", "var(--color-teal)", "var(--color-artel)"];
...
export function SceneHero({ active }: { active: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="drift pointer-events-none absolute left-[6%] top-[22%] hidden h-12 w-12 border-[3px] border-[var(--color-artel)] bg-[var(--color-artel)] lg:block" style={{ animationDelay: "0s" }} aria-hidden />
      ...
      {/* Floating 3D ROSCA art behind the card */}
      <div className="float-y pointer-events-none absolute right-[2%] top-[14%] -z-10 hidden w-[clamp(220px,22vw,360px)] opacity-20 lg:block" aria-hidden>
        <Image src="/rosca-3d.png" alt="" width={360} height={360} className="h-auto w-full object-contain" />
      </div>
      ...
      <div className="grid w-full max-w-5xl grid-cols-1 items-center gap-10 pt-24 pb-16 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div>
          ...
          <h1 className="font-display mt-5 leading-[0.82]">
            ...
            <span
              ...
              style={{ color: "var(--color-artel)", transitionDelay: "200ms" }}
            >
              {h.titleLine2}
            </span>
            <span className="scene-item mt-1 flex items-center gap-3 text-[9vw] sm:text-[3.5rem]" style={{ transitionDelay: "280ms" }}>
              ...
              <span className="inline-block border-[3px] border-[var(--color-text)] bg-[var(--color-text)] px-3 py-0.5 text-[var(--color-artel)] brutal-shadow">
                Stellar
              </span>
            </span>
          </h1>

          <div className="scene-item mt-6 max-w-md border-l-[5px] border-[var(--color-artel)] pl-4" style={{ transitionDelay: "360ms" }}>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <span className="absolute -top-3 right-5 border-[3px] border-[var(--color-text)] bg-[var(--color-artel)] px-2 py-0.5 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--color-ink)]">
              {h.builtOn}
            </span>
            ...
            <div className="relative mx-auto flex aspect-square w-full max-w-[230px] items-center justify-center">
              ...
              <div className="flex aspect-square w-[88%] items-center justify-center rounded-full border-[4px] border-[var(--color-text)] bg-[var(--color-artel)]">
                <span className="grid h-[80%] w-[80%] place-items-center rounded-full bg-[var(--color-surface)]">
                  <ArtelLogo size={170} className="h-[88%] w-[88%]" />
                </span>
              </div>
            </div>
          ...
          <div className="scene-item grid grid-cols-2 gap-3" style={{ transitionDelay: "360ms" }}>
            <div className="border-[3px] border-[var(--color-text)] bg-[var(--color-surface)] p-3 brutal-shadow">
              ...
              <p className="font-display text-2xl" style={{ color: "var(--color-artel)" }}>{h.stats[0].value}</p>
            </div>
            <div className="border-[3px] border-[var(--color-text)] bg-[var(--color-surface)] p-3 brutal-shadow">
          ...
          <div className="scene-item border-[3px] border-[var(--color-text)] bg-[var(--color-text)] p-3 brutal-shadow" style={{ transitionDelay: "440ms" }}>
            ...
            <p className="text-sm font-black uppercase tracking-wide text-[var(--color-surface)]">
              <span style={{ color: "var(--color-artel)" }}>100%</span> {h.stats[2].value.replace("100% ", "")}
            </p>
          </div>
        </div>
      </div>
