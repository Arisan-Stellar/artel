...
export function Tempaan() {
  ...
  return (
    <section ref={ref} id="tempaan" className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 py-24 pb-40 sm:pb-48 text-center">
        ...
        <div
          className="mt-6 mx-auto max-w-lg border-[3px] border-[var(--color-text)] bg-[var(--color-surface)] px-6 py-4 text-center brutal-shadow"
          style={revealItem(progress, 0.18, 0.34)}
        >
          <p className="font-num text-sm font-bold uppercase tracking-wider text-[var(--color-text)]">{dict.tempaan.formula}</p>
          <p className="mt-2 text-xs font-semibold text-[var(--color-muted)]">
            {safe ? (
              <>If the pool pays <strong className="text-[var(--color-teal)]">100 XLM</strong>, fleeing costs you <strong className="text-[var(--color-crack)]">125 XLM</strong> in collateral — a guaranteed net loss of <strong className="text-[var(--color-crack)]">25 XLM</strong>.</>
            ) : (
              <>At {value}% collateral, the numbers still favor the runner. Keep scrolling to see when it becomes unprofitable.</>
            )}
          </p>
        </div>
      </div>
